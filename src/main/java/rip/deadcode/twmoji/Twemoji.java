...
public final class Twemoji {
    ...
    public String parse(String text, TwemojiOption options) {
        return replace(text, raw -> {
            int iconId = grabIcon(text);
            ...
            if (Strings.isNullOrEmpty(src)) {
                ...
                for (Map.Entry<String, String> attributePair : attributes.entrySet()) {
                    if (!attributePair.getKey().contains("on") && sb.indexOf(attributePair.getKey() + "=") == -1) {
                        sb.append(" ").append(attributePair.getKey()).append("=").append(escapeHtml(attributePair.getValue()));
                    }
                }
                sb.append("/>");
                return sb.toString();
    ...
    private static int grabIcon(String raw) {
        return toCodePoint(raw.contains("\u200d") ? raw.replace("\ufe0f", "") : raw)[0];
    }
    ...
    public String replace(String text, Function<String, String> replacer) {
